...
# хотел сделать возможность выбрать язык, но это оказалось затруднительно(
# translations = {
#     'en': {
#         'grretings': "Welcome to the game 'Battleship of the century'!",
#         'instructions': "The game board is {}x{} cells.",
#         'enter_coordinates': "Your move (format 'x y'): ",
#         'out_of_bounds': "Coordinates are out of the game board. Try again.",
#         'congratulations': "Congratulations! You have sunk all the ships!",
#         'invalid_input': "Invalid input. You need to enter two numbers separated by space.",
#         'hit': "ship got shot!",
#         'miss': "Miss!",
#         'sunk': "ship is *_*",
#         'already_shot': "already shot here!"
#     },
#     'ru': {
#         'grretings': "Добро пожаловать в игру 'Морской бой'!",
#         'instructions': "Игровое поле имеет размер {}x{} клеток.",
#         'enter_coordinates': "Ваш ход (формат 'x y'): ",
#         'out_of_bounds': "Координаты выходят за пределы игрового поля. Попробуйте еще раз.",
#         'congratulations': "Поздравляем! Вы потопили все корабли!",
#         'invalid_input': "Некорректный ввод. Нужно ввести два числа через пробел.",
#         'hit': "попадание!",
#         'miss': "промах!",
#         'sunk': "корабль потопулся...",
...
# при желании цвета можно легко поменять не колупаясь во всей логике
class Color:
    yellow2 = '\033[1;35m'
    reset = '\033[0m'
    blue = '\033[0;34m'
                ...
                for y in range(self.size):
                    ...
                    for rotation in range(0, 4):
                        ...
                        if self.check_ship_fits(ship, FieldPart.radar):
                            self.weight[x][y] += 1
class Game(object):
  #сделал поле чуть больше, а так же добавил еще пару кораблей для разнообразия 
        ...
        for ship_size in Game.ships_rules:
            ...
            while True:
                ...
                if player.auto_ship_setup is not True:
                    ...
                    print('{}. Loading...'.format(player.name))
...
if __name__ == '__main__':
    ...
    while True:
        ...
        if game.status == 'in game':
            ...
            Game.clear_screen()
            game.current_player.message.append("Куда стреляем, сэр? ")
            game.draw()
            ...
            if shot_result == 'miss':
                ...
                game.next_player.message.append('Плохие новости, капитан, наш корабль был уничтожен :(')
                continue
